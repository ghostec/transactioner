...
func main() {
	...
	pgc := &pg.Client{Pool: &mockedPool{}}
	tr := transactioner.Transactioner(pgc)

	uc := usecase{Transactioner: tr, repo: &repository{querier: pgc}}
	if err := uc.operationThatMustBeAtomic(ctx); err != nil {
		panic(err)
	}
}

func (uc *usecase) operationThatMustBeAtomic(ctx context.Context) (dst error) {
